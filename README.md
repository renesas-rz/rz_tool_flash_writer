# README of the RZ/G3L DDR Tools
...
The RZ/G3L DDR Tools, based on the RZ/G3L Flash Writer, are essential utilities provided by Renesas to help developers configure, tune, and verify LPDDR4 memory operation on the RZ/G3L MPU.

## 1. Overview

This document explains the RZ/G3L DDR Tools for Renesas RZ/G3L MPU, which is based on the RZ/G3L Flash Writer.

The RZ/G3L DDR Tools is downloaded from the Host PC via SCIF by boot ROM.
...
And the RZ/G3L DDR Tools downloads the some of the raw images from Host PC via SCIF, and writes the raw images to the Serial NOR Flash (hereafter referred to as “Serial Flash”).

[Chapter 2](#2-operating-environment) describes the operating environment.

[Chapter 3](#3-software) describes the software.

[Chapter 4](#4-how-to-build-the-rzg2-flash-writer) explains example of how to build the RZ/G3L DDR Tools.

[Chapter 5](#5-how-to-run-the-rzg2-flash-writer) explains example of how to perform the RZ/G3L DDR Tools.
...
The RZ/G3L DDR Tools is distributed as a sample software from Renesas without any warranty or support.
...
| Name         | Note                                              |
| ------------ | ------------------------------------------------- |
| Target board | RZ/G3L SMARC Evaluation Kit (RZ/G3L EVK)          |
| Host PC      | Ubuntu Desktop 22.04(64bit) or later              |

The following table shows Serial Flash and eMMC support for RZ/G3L EVK.

#### Serial Flash / eMMC support status of RZ/G3L EVK
...
The RZ/G3L DDR Tools support the following build options.

#### 3.2.1. BOARD

| BOARD            | BOARD setting                                                 |
| ---------------- | ------------------------------------------------------------- |
| RZG3L_SMARC      | Generate binary that works on RZ/G3L EVK board. (default)     |
...
| --------------------------- | ------------------- | ------------------ | ------------------------------------------- |
| bl2_bp_spi-smarc-rzg3l.srec | H'21E00             | H'00000            | Loader                                      |
| fip-smarc-rzg3l.srec        | H'00000             | H'60000            | ARM Trusted Firmware and U-boot in FIP file |

The following shows the procedure of this command.

```text
>XLS2
===== Qspi writing of RZ/G3L Board Command =============
...
======= Qspi  Save Information  =================
 ...
 SpiFlashMemory End Address  : H'0001E7F7
===========================================================
```
...
| ---------------------------- | ------------------- | ----------------- | ------------------- | ------------------------------------------- |
| bl2_bp_emmc-smarc-rzg3l.srec | boot partition1     | H'00000001        | H'21E00             | Loader                                      |
| fip-smarc-rzg3l.srec         | boot partition1     | H'00000300        | H'00000             | ARM Trusted Firmware and U-boot in FIP file |
...
Please Input Program Start Address : 21E00                  <<<< Enter "21E00" here
...
```
Input file size with 0x11E28 = 73,256 bytes
...
## 4. How to build the RZ/G3L DDR Tools

This chapter is described how to build the RZ/G3L DDR Tools.
...
### 4.3. Build the RZ/G3L DDR Tools

S-record file will be built by the following command.

```bash
$ cd ${DDR_TOOLS_DIR}/rz_tool_flash_writer/scripts/
$ ./build_flash_writer_g3l.sh RZG3L_SMARC
```

> [!NOTE]
>
> DDR features and DDR debug log (DDR debug log is used to run the Eye Opening Tool) are enabled by default in `build_flash_writer_g3l.sh`.
>
> To disable them, set `DDR_PARAMLOAD=DISABLE` and `DDR_DEBUG=0` in the build script (`build_flash_writer_g3l.sh`), or remove these options before building the RZ/G3L DDR Tools.

Output image will be available in the following directory:

* ${DDR_TOOLS_DIR}/rz_tool_flash_writer/AArch64_output/Flash_Writer_SCIF_RZG3L_SMARC_LPDDR4X.mot
...
```bash
$ cd ${DDR_TOOLS_DIR}/rz_tool_flash_writer/scripts/
$ ./g3x_c2bin.sh ${INSTALL_UTILITY_DIR}/rz_tool_flash_writer/soc/g3l/board/smarc/ddr_param_def_lpddr4.c
```

A binary file will be generated at the following path:
* ${DDR_TOOLS_DIR}/rz_tool_flash_writer/scripts/ddr_param_def_lpddr4.bin

## 5. How to run the RZ/G3L DDR Tools

### 5.1. Prepare for run DDR Tools

Start the target in the SCIF download mode and run the RZ/G3L DDR Tools sample code.
...
- AArch64_output/Flash_Writer_SCIF_RZG3L_SMARC_LPDDR4X.mot

After the transfer has succeeded, the following log will be shown.

```text
Flash writer for RZ/G3L Series Vx.xx MMM.DD,YYYY
 Product Code : RZ/G3L
>
```
...
Describe the revision history of RZ/G3L DDR Tools.
