...
#define	EMMC_WORK_DRAM_SADD	INTERNAL_WORK_MEMORY_SADD
#define	EMMC_WORK_DRAM_EADD	INTERNAL_WORK_MEMORY_EADD
#else
#define	EMMC_WORK_DRAM_SADD	DDR_WORK_MEMORY_SADD
#define	EMMC_WORK_DRAM_EADD	DDR_WORK_MEMORY_EADD
#endif

#define	EMMC_WORK_DRAM_SECTOR_MAX	((EMMC_WORK_DRAM_EADD - EMMC_WORK_DRAM_SADD + 1)>>9)
...
void dg_emmc_write(EMMC_WRITE_COMMAND wc)
{
	...
	static const int8_t endMessage[][32] =	{
		"EM_W Complete!",
		"EM_WB Complete!"
	};
...
//Select Partition
	...
	switch(partitionArea)
	{
		case EMMC_PARTITION_USER_AREA:	//User Partition Area Program
			Load_workEndAdd = (uint32_t*)EMMC_WORK_DRAM_EADD;
...
// WorkMemory CLEAR (Write H'00000000)
	PutStr("Work RAM (H'",0);
	Data2HexAscii((uintptr_t)Load_workStartAdd, str, 4);
	PutStr(&str[0], 0);
	PutStr("-H'", 0);
	Data2HexAscii((uintptr_t)Load_workEndAdd, str, 4);
	PutStr(&str[0], 0);
	PutStr(") Clear....",1);
