...
ifeq ("$(DDR_PARAM_LOAD)", "ENABLE")
	CFLAGS += -DDDR_PARAM_LOAD=1
	STACK_BYTES := 24576
else
	CFLAGS += -DDDR_PARAM_LOAD=0
	STACK_BYTES := 4096
endif
...
$(OBJECT_DIR)/%.o:$(BOOTDIR)/%.s
	$(AS)  -g $(CPU) $(AS_NEON) --MD $(patsubst %.o,%.d,$@) -I $(BOOTDIR) -I $(INCLUDE_DIR) -I $(DDR_COMMON) -I $(DDR_SOC) $< -o $@ --defsym $(AArch32_64)=0 --defsym $(BOOT_DEF)=0 --defsym $(TOOL_DEF)=0 --defsym STACK_BYTES=$(STACK_BYTES)
