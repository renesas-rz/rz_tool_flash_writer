...
{
	uint32_t mail = 0, data = 0, train_done = 0, mail_code = 0, mail_argv = 0;
	...
	while (train_done == 0) {
		...
		if ((data & 0x1) == 0) {
			...
			if (mail == 0x08) {
				mail_code = dwc_ddrphy_get_mail(1);
#ifdef DDR_DEBUG
				LOG("%08x\r\n", mail_code);
#endif // DDR_DEBUG

				uint16_t mail_argc = (uint16_t)(mail_code & 0xFFFFU);
				for (uint16_t i = 0; i < mail_argc; i++) {
					mail_argv = dwc_ddrphy_get_mail(1);
#ifdef DDR_DEBUG
					LOG("%08x\r\n", mail_argv);
#endif // DDE_DEBUG
				}
