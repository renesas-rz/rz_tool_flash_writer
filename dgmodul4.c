...
#if INTERNAL_MEMORY_ONLY == 1
//////////////////////////////////////////
// XLoadSpiflash0_4
//////////////////////////////////////////
static void XLoadSpiflash0_2(void)
{
	char		str[64];
	...
	ParameterSectorEraseQspiFlash(PrgSpiStartAdd, PrgSpiStartAdd + PrgSpiSize - 1);
	// SAVE QSPI-FLASH
...
void dgG2LoadSpiflash0_3(void)
{
	char		str[64];
	char 		bin_data;
	uint32_t	PrgSpiStartAdd, PrgSpiSize, spiAddr;
	uint32_t 	loop = 0;
	...
	spiAddr = PrgSpiStartAdd;
	while (loop < PrgSpiSize)
	{
		GetChar(&bin_data);
		*ptr++ = bin_data;
		loop++;
		...
		if (readCount == 256)
		{
			...
			PageProgramWithBuffeQspiFlashNoWait(spiAddr, LS_WORK_DRAM_SADD + (256 * bufferIndex));
			spiAddr += 256;
			if (bufferIndex == 0)
			{
				bufferIndex = 1;
				ptr = (unsigned char *)(LS_WORK_DRAM_SADD + 256);
			}
			else
			{
				bufferIndex = 0;
...
static char dgLS_Load_And_Write_LowMemory(uint32_t spiStartAdd, uint32_t *spiEndAdd, uint32_t prgStatAdd)
{
	...
	uint32_t	data;
	uint32_t	getByteCount, byteCount;
	uint32_t	loadGetCount,adByteCount,loadGetData,loadGetCR;
	uint32_t	loadGetAddress, prevLoadAddress;
	uint32_t	loop, loop_S0, s0flag,errFlg, checkData,endFlg;
	uint32_t	readCount = 0;
	uint32_t	prgSpiStartAdd;
	uint32_t	bufferIndex = 0;
	unsigned char	*ptr;
	uint32_t status;

	prevLoadAddress = prgStatAdd;
	ptr 	        = (unsigned char *)LS_WORK_DRAM_SADD;
	prgSpiStartAdd  = spiStartAdd;
	...
	while(loop)
	{
		...
		if (endFlg == 1 || errFlg == 1)
		{
			while(1)
			{
				...
				if ((*str == CR_CODE) || (*str == LF_CODE))
				{
					if (readCount)
					{
						SaveDataWithBuffeQspiFlash(LS_WORK_DRAM_SADD + (bufferIndex * 256), prgSpiStartAdd, readCount);
					}
					*spiEndAdd = prgSpiStartAdd += readCount - 1;
					return(0);
				}
			}
		}
		if (s0flag == 0)
		{
			...
			if (prevLoadAddress != loadGetAddress)
			{
				...
				if (readCount)
				{
					addr = LS_WORK_DRAM_SADD + (bufferIndex * 256) + readCount;
					memset((void *)addr, 0xff, (256 - readCount));
					PageProgramWithBuffeQspiFlashNoWait(prgSpiStartAdd, LS_WORK_DRAM_SADD + (bufferIndex * 256));
				}
				prgSpiStartAdd += readCount;
				prgSpiStartAdd += (loadGetAddress - prevLoadAddress);
				if (bufferIndex == 0)
				{
					bufferIndex = 1;
					ptr = (unsigned char *)(LS_WORK_DRAM_SADD + 256);
			...
			for (byteCount = loadGetCount; loadGetCount > 1; loadGetCount = loadGetCount - 1)
			{
				...
				*ptr++ = (unsigned char)data;
				...
				if (readCount == 256)
				{
					...
					PageProgramWithBuffeQspiFlashNoWait(prgSpiStartAdd, LS_WORK_DRAM_SADD + (256 * bufferIndex));
					prgSpiStartAdd += 256;
					if (bufferIndex == 0)
					{
						bufferIndex = 1;
